...
package org.herrlado.websms.connector.arcor;
...
public class ConnectorArcor extends Connector {
    ...
    private static final Pattern BALANCE_MATCH_PATTERN = Pattern.compile(
            "<td.+class=\"txtRed\".*?<b>" + "(\\d{1,2})</b>.+"
                    + "<td.+class=\"txtRed\".*?<b>(\\d{1,})</b>", Pattern.DOTALL);
    ...
    public final ConnectorSpec initSpec(final Context context) {
        ...
        c.setCapabilities(ConnectorSpec.CAPABILITIES_UPDATE | ConnectorSpec.CAPABILITIES_SEND
                | ConnectorSpec.CAPABILITIES_PREFS | ConnectorSpec.CAPABILITIES_LIMITLENGTH_8BIT);
        c.addSubConnector("arcor.de", c.getName(), 0);
        return c;
    }
     ...
     * Login to arcor.
     * 
     * @param ctx {@link ConnectorContext}
     * @return true if successfullu logged in, false otherwise.
     * @throws WebSMSException if any Exception occures.
     */
    private boolean login(final ConnectorContext ctx) throws WebSMSException {
        try {
            ...
            final HttpPost request = createPOST(
                    LOGIN_URL,
                    getLoginPost(p.getString(Preferences.USER, ""),
                            p.getString(Preferences.PASSWORD, "")));
            final HttpResponse response = ctx.getClient().execute(request);
            final String cutContent = cutLoginInfoFromContent(response.getEntity().getContent());
     ...
     * 
     * @param ctx {@link ConnectorContext}
     * @throws WebSMSException on an error
     */
    private void updateBalance(final ConnectorContext ctx) throws WebSMSException {
        try {
            final HttpResponse response = ctx.getClient().execute(new HttpGet(SMS_URL));
     ...
     * 
     * @param ctx {@link ConnectorContext}
     * @param content conten to investigate.
     */
    ...
    private boolean sendSms(final ConnectorContext ctx) throws WebSMSException {
        try {
            ...
            int limit = 135; // das ist default. aber falls man die registrierte
                             // Nummer als Absender verwendet, kann man maximal
                             // 160 Zeichen verschicken.
            if (ctx.getPreferences().getBoolean(Preferences.ENABLE_VALIDATED_NUMBER, Boolean.FALSE)) {
                limit = 160;
            }
            ...
            if (text.length() <= limit) {
                response = ctx.getClient().execute(createPOST(SMS_URL, this.getSmsPost(ctx, text)));
                return this.afterSmsSent(ctx, response);
            }

            if(ctx.getPreferences().getBoolean(Preferences.ENABLE_CHAIN_SMS, false) == false){
                throw new WebSMSException(ctx.getContext().getString(R.string.text_too_long));
            }
            ...
            for (int i = 0; i < smscount; ++i) {
                ...
                response = ctx.getClient().execute(createPOST(SMS_URL, this.getSmsPost(ctx, text.substring(start, end))));
                sent &= this.afterSmsSent(ctx, response);
            }
            if(lastsms > 0){
                response = ctx.getClient().execute(createPOST(SMS_URL, this.getSmsPost(ctx, text.substring(end))));
                sent &= this.afterSmsSent(ctx, response);
     ...
     */
    private boolean afterSmsSent(final ConnectorContext ctx, final HttpResponse response)
            throws Exception {

        final String body = cutFreeCountFromContent(response.getEntity().getContent());
        ...
        if (!found || m.groupCount() != 2) {
            ...
            throw new Exception(ctx.getContext().getString(R.string.log_unknow_status_after_send));
        }
     ...
     * @throws UnsupportedEncodingException if the url can not be encoded
     */
    private static String getLoginPost(final String username, final String password)
        ...
        sb.append("&login=Login&protocol=" + "https&info=Online-Passwort&goto=");
        return sb.toString();
    }
     ...
     * @throws Exception if an error occures.
     */
    private String getSmsPost(final ConnectorContext ctx, String body) throws Exception {
        final StringBuilder sb = new StringBuilder();
        ...
        sb1.append(URLEncoder.encode(body, ARCOR_ENCODING));
        sb1.append("&firstVisitOfPage=foo&part=0&senden=Senden" + "&ordnername=Posteingang");

        if (ctx.getPreferences().getBoolean(Preferences.COPY_SENT_SMS, Boolean.TRUE)) {
        ...
        if (ctx.getPreferences().getBoolean(Preferences.ENABLE_VALIDATED_NUMBER, Boolean.FALSE)) {
            ...
            String email = ctx.getPreferences().getString(Preferences.USER, "") + "@arcor.de";
     ...
     * @param urlencodedparams key=value pairs as url encoded string
     * @return HttpPost
     * @throws Exception if an error occures
     */
    private static HttpPost createPOST(final String url, final String urlencodedparams)
            throws Exception {
